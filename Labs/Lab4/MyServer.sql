drop table persons; create table persons( person_id integer, person_name varchar(20), person_surname varchar(20), person_age integer not null, person_wealth integer, person_weight float ); 
drop table jobs_person; create table jobs_person( jobs_id integer, person_id integer, start_date date, end_date date); 
drop table jobs; create table jobs( jobs_id integer, job_description varchar(200), salary integer ); 
...
ALTER TABLE
   persons
ADD CONSTRAINT
   c1 PRIMARY KEY (person_id);
   
SELECT person_id, count(persons_id)
FROM persons
GROUP BY persons_id
having count (persons_id) > 1;

select * from persons;

select * from persons where person_id>1000 and person_id<3000;

delete from persons where person_id = 1001;
