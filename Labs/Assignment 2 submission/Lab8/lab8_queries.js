...
cursor.forEach(function (cursor) {
    ...
    db.d14123580_teams.save(cursor);
});

// q 6. update the points of Arsenal to be equal to the point of Barcelona

var findPoints = db.d14123580_teams.findOne({ name: 'Barcelona' }).points;
print(toPoint);
db.d14123580_teams.update(
    {name : "Arsenal" },
...
function old_vs_young(x){
    ...
    {$group: {_id: '$players.p_id', 
        goalPerAge : {'$players.age' : {$gte : x}, 
            {$first : '$players.goal'}, {$first : 1}},
        lessThanAge : {'$players.age' : {$lte : x}, 
            {$first : '$players.goal'}, {$first : 0}}}}
    );
}
