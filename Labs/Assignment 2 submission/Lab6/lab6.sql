...
insert into Player_Stage (sourcedb,p_id,player_name) 
select 1,p_id,p_name from Players1;

insert into Player_Stage (sourcedb,p_id,player_name) 
select 2,p_id,p_name from Players2;
...
insert into Tournament_Stage (sourcedb,T_id,tournament_desc, total_price) 
select 1,T_id,tournament_desc, total_price from Tournament1;

insert into Tournament_Stage (sourcedb,T_id,tournament_desc, total_price) 
select 2,T_id,tournament_desc, total_price from Tournament1;
...
insert into year_stage(day) select distinct cast(to_char(t_date,'YYYY') as integer) from tournaments1;
insert into year_stage(month) select distinct cast(to_char(t_date,'MM') as integer) from tournaments1;
insert into year_stage(day) select distinct cast(to_char(t_date,'DD') as integer) from tournaments1;
insert into year_stage(quater) select distinct cast(to_char(t_date,'Q') as integer) from tournaments1;
...
/* Dimension Player */
insert into Player_Stage (sourcedb,p_id,player_name) 
select 1,p_id,player_name from players1 where
NOT EXISTS (SELECT * FROM Player_Stage ps
              WHERE  ps.p_id= players1.p_id and ps.sourcedb=1 );  /* updating from database 1 */

insert into Player_Dim select p_sk,p_name from Player_Stage ps where
NOT EXISTS (SELECT * FROM Player_Dim
              WHERE  ps.p_sk= Player_Dim.p_sk );  /* updating only new entities */
          
/* Dimension Teams */
insert into Team_Stage (sourcedb,Team_id,team_name) 
select 1,Team_id,team_name from teams1 where
NOT EXISTS (SELECT * FROM Team_Stage ts
              WHERE  ts.Team_id= teams1.Team_id and ts.sourcedb=1 );  /* updating from database 1 */

insert into Team_Dim select Team_id,team_name from Team_Stage ts where
NOT EXISTS (SELECT * FROM Team_Dim
              WHERE  ts.Team_id= Team_Dim.Team_id );  /* updating only new entities */
              
/* Dimension Tournaments */
insert into Tournament_Stage (sourcedb,p_id,tournament_desc, total_price) 
select 1,t_id,tournament_desc, total_price from tournaments1 where
NOT EXISTS (SELECT * FROM Tournament_Stage dt
              WHERE  dt.T_id= tournaments1.T_id and dt.sourcedb=1 );  /* updating from database 1 */

insert into Tournament_Dim select _id,tournament_desc, total_pricer from Tournament_Stage cs where
NOT EXISTS (SELECT * FROM Tournament_Dim
              WHERE  dt.T_id= Tournament_Dim.T_id );  /* updating only new entities */

/* Dimension Date */
insert into Date_Stage (sourcedb,d_id,day,month,year,week,quater,dayofweek) 
select 1,T_id,t_date from tournaments1 where
NOT EXISTS (SELECT * FROM Date_Stage dd
              WHERE  dd.T_id= tournaments1.t_date and dd.sourcedb=1 );  /* updating from database 1 */

/* Normalize new data */
update Date_Stage set day='Monday' where day='1';
update Date_Stage set day='Tuesday' where day='2';
update Date_Stage set day='Wednesday' where day='3';
update Date_Stage set day='Thursday' where day='4';
update Date_Stage set day='Friday' where day='5';
update Date_Stage set day='Saturday' where day='6';
update Date_Stage set day='Sunday' where day='7';

insert into Date_Dim select d_id,day,month,year,week,quater,dayofweek from Date_Stage ds where
NOT EXISTS (SELECT * FROM Date_Dim
              WHERE  ds.d_id= Date_Dim.d_id );  /* updating only new entities */